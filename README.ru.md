...
В `config.yaml` можно установить значение для полей:
...
- `sample_rate` - определяет частоту дискретизации распознаваемого аудиофайла. Доступность частоты ограничена выбранной моделью Vosk. Для `vosk-model-small-ru-0.22` рекомендуется распознавать аудио с частотой 16000(Гц).
...
При использовании собственного аудиофайла убедитесь, что он имеет правильный формат:
- Для WAV - PCM_S16LE mono (частотой 16000Гц)
- Для RAW - PCM_S16LE / PCM_S16BE mono (частотой 16000Гц)
...
5) А так же может потребоваться установка [ONNX runtime](https://onnxruntime.ai/docs/install/).
